...
Describe "Drift Detection Logic" {
    ...
    Context "Identical snapshots" {
        It "reports zero drift when current state matches checkpoint" {
            # NEEDS AUTHORING:
    ...
    Context "Changed value" {
        It "detects drift when a control value has regressed" {
            # NEEDS AUTHORING:
    ...
    Context "Missing key in current state" {
        It "handles a key present in checkpoint but absent in current state" {
            # NEEDS AUTHORING:
